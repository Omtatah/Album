...
class Image(models.Model):
    ...
    def search_by_category(cls, search_term):
        searched_images = cls.objects.filter(
            image_category__category__icontains=search_term)
        return searched_images

    @classmethod
    def filter_by_location(cls, location_filter):
        located_images = Image.objects.filter(
            image_location__id=location_filter)
